...
Detailed description of each configuration field:

- `request_path_regex` - regexes matched against the request path to decide if the plugin should run.
- `request_body_regex` - regexes matched against the raw request body; must match along with `request_path_regex`.
- `prompt_suffix` - extra instructions appended to the system prompt sent to the model.
- `model` - the LLM to use (`gpt-4o`, `gpt-4o-mini`, `gpt-4o-2024-05-13`, `gpt-4o-mini-2024-07-18`, `gemini-2.0-flash`, `gemini-2.5-flash-preview-04-17`, `claude-3-7-sonnet-latest`, `ollama`).
- `temperature` - controls randomness in the model output; lower values yield more deterministic replies.
- `max_tokens` - maximum number of tokens generated by the model; 150 is usually enough for spam checks.
- `api_key_openai` - API key used when selecting an OpenAI model.
- `api_key_google` - API key required for Google Gemini models.
- `api_key_anthropic` - API key required for Anthropic Claude models.
- `ollama_url` - URL of the local Ollama API endpoint, for example `http://localhost:11434/api/generate`.
- `ollama_model` - model name for Ollama, for example `gemma3:latest`.
- `action` - behaviour when spam is detected: `block-if-spam-or-error`, `block-if-spam-only`, `log-if-spam-or-error`, `log-if-spam-only`.
- `custom_response_code` - HTTP status code used when blocking a request, default `403`.
- `custom_response_body` - body template for blocked responses. Supports `%{result.category}`, `%{result.reason}` and `%{plugin_conf.model}` placeholders.
- `custom_response_content_type` - `Content-Type` header returned with the custom response.
- `custom_response_cache_control` - `Cache-Control` header for blocked responses, defaults to `max-age=0, private, no-store, no-cache, must-revalidate`.
